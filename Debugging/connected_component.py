...
def check_neighbours(row, column, color):
    for k in range(4):
        next_row = row + [1, -1, 0, 0][k] 
        next_col = column + [0, 0, 1, -1][k]
        if next_row > len(matrix)-1 or next_col > len(matrix[row])-1 or next_row < 0 or next_col < 0:
            continue
        if matrix[next_row][next_col] != 1:
            continue
        matrix[next_row][next_col] = color
        check_neighbours(next_row, next_col, color)
